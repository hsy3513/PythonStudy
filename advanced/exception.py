...
def practice3():
  ...
  #    get[키값]        : 필수 항목인 경우, 반드시 데이터가 존재해야 하는 경우, 예외 처리를 해야 하는 경우
  try:
...
class NegativerNumberError(Exception):
  """ 음수 입력 예외 """
  def __str__(self):
...
def practice4():
  ...
  try:
    ...
    print(f'입력값: {n}')
    if n < 0:
      raise NegativerNumberError
  except ValueError:
    print('숫자만 입력하세요.')
  except NegativerNumberError as e:
    # print(e)
