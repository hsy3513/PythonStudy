...
def practice2():
  ...
  # 홀수의 값은 원래 값 그대로..
  data = [ x**2 if x % 2 == 0 else x for x in numbers ]
  print(data)

  words = ['animals', 'foods', 'tools', 'happy', 'sad']
  # 위 단어들 중 글자수가 5자인 항목만 대문자로 변경
  # 조건에 해당되지 않는 경우 원래 값 그대로 사용
  trans_words = [ i.upper() if len(i) == 5 else i for i in words]
  print(trans_words)
...
def practice3():
  ...
  sentences = []
  while True:
    stc = input('문장을 입력하세요..(종료: -1): ').strip()
    if stc == '-1':
      break
  ...
  # 조건에 맞게 리스트 재구성(전처리)
  trans_stc = [ s.strip().lower() for s in sentences if s != '']
...
def practice4():
  ...
  # images = [ s.replace(".png", "").replace(".jpg", "").replace(".jpeg", "") for s in files ]
...
def practice5():
  # 딕셔너리로 데이터 변환
  # : (상품, 가격) 리스트에서 {상품명: 가격 + VAT(부가가치세)} 딕셔너리 생성
  products = [
    ('keyboard', 40000),
    ('mouse', 20000),
    ('monitor', 100000),
